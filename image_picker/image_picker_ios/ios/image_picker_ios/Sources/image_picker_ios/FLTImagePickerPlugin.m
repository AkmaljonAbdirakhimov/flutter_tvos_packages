...
// Import PhotosUI conditionally as it's not fully available on tvOS
#if !TARGET_OS_TV
...
#if TARGET_OS_TV
  // PHPicker is not fully supported on tvOS
  [self sendCallResultWithError:[FlutterError errorWithCode:@"photo_picker_not_available"
...
- (void)pickMultiImageWithMaxSize:(nonnull FLTMaxSize *)maxSize
  ...
  if (@available(iOS 14, *)) {
#if TARGET_OS_TV
    // On tvOS, use UIImagePickerController since PHPicker isn't fully supported
...
- (void)showPhotoLibraryWithPHPicker:(PHPickerViewController *)pickerViewController
    API_AVAILABLE(ios(14)) {
#if TARGET_OS_TV
  // This method should not be called on tvOS, but adding this as a safeguard
  [self sendCallResultWithError:[FlutterError errorWithCode:@"photo_picker_not_available"
                                                    message:@"PHPicker is not available on tvOS"
                                                    details:nil]];
#else
